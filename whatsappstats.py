...
def max_length_msg(filename):
    ...
    for msg in df['Message']:
        if(i < len(str(msg).split(' '))):
            if(df[df['Message']==msg]['Contact'].shape[0]>0):
                i = len(str(msg).split(' '))
                name = df[df['Message']==msg]['Contact'].values[0]
    return (i,name)
...
def most(filename):
    ...
    sorted_active = df.groupby('Contact').count()['Time'].sort_values()  
    if(df.groupby('Contact').count().shape[0]>10):
...
'''def least(filename):
    ...
    sorted_active = df.groupby('Contact').count()['Time'].sort_values()  
    if(df.groupby('Contact').count().shape[0]>5):
...
'''def pie(filename):
    ...
    plt.title("Message in respective Meridian", fontsize=20)
